// FIX Bug31: escape helper para renderMotivBanner y otros innerHTML en este archivo
function _ep(s){if(s==null)return"";return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");}
...
function renderMotivBanner(pid){
  ...
  // FIX Bug34: normalizar valores de adh por si tienen formato objeto tras sync Supabase
  const _npv=v=>{if(!v)return"none";if(typeof v==="string")return v;if(typeof v==="object")return v.notes||"none";return"none";};
  const doneDays=Object.values(adh).filter(v=>_npv(v)==="done").length;
  ...
  if(streak>=14){
    ...
    if(sorted.length>=2){
      // FIX Bug3: usar dirección del objetivo para mostrar progreso tanto en pérdida como en ganancia
      const _dir=weightGoalDir(pt?.goal,pt?.ficha);
      const _rawDiff=parseFloat((sorted[sorted.length-1].weight-sorted[0].weight).toFixed(1)); // newest - oldest (positivo = ganancia)
      const _lossAmt=parseFloat((sorted[0].weight-sorted[sorted.length-1].weight).toFixed(1)); // positivo = bajó
      const _gainAmt=_rawDiff; // positivo = subió
      if(_dir==="up"){
        // Paciente con meta de volumen: mostrar ganancia
        if(_gainAmt>=3){
          type="progress";emoji="💪";title=`+${_gainAmt} kg ganados desde el inicio`;
          sub="Masa muscular que se traduce en fuerza real.";
        } else if(_gainAmt>=1){
          type="progress";emoji="📈";title=`+${_gainAmt} kg desde el inicio`;
          sub="Cada kilo ganado con disciplina vale doble.";
        } else if(doneDays>=10){
          type="progress";emoji="💎";title=`${doneDays} días de entrenamiento acumulados`;
          sub="La constancia es tu superpoder.";
        } else {
          type="start";emoji="🎯";title=`Semana ${pt.week||1} — ${_ep(pt.goal||"Objetivo activo")}`; // FIX: escapar pt.goal
          sub="Registra tu peso y sube a la siguiente semana.";
        }
      } else {
        // Paciente con meta de pérdida o recomposición (default)
        const diff=_lossAmt;
        if(diff>=3){
      ...
      type="start";emoji="👋";title=`Bienvenido a tu espacio, ${_ep(pt.name.split(" ")[0])}`; // FIX Bug31
