// FIX Bug32+35+36: escape helper para innerHTML en este archivo
function _el(s){if(s==null)return"";return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");}
...
function renderAdmTimeline(pid,showAll=false){
  ...
  progSorted.forEach((e,i)=>{
    ...
    const note=e.note?` · ${_el(e.note)}`:""; // FIX Bug32: escapar nota de pesaje
...
function renderSchedList(pid){
  ...
  // FIX Bug35: escapar n.msg + usar data-id en lugar de pid inline en onclick
  wrap.innerHTML=arr.slice().reverse().map(n=>`
    <div class="sched-item ${n.delivered?"delivered":""}">
      <div class="sched-date">${n.date}</div>
      <div class="sched-msg">${_el(n.msg)}</div>
      ${n.delivered?`<span style="font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:800;color:var(--green);letter-spacing:1px">Enviado</span>`:`<div class="sched-del" data-sched-del="${_el(n.id)}">✕</div>`}
    </div>`).join("");
  // Delegación de eventos — pid nunca toca el HTML
  wrap.querySelectorAll("[data-sched-del]").forEach(el=>{
    el.addEventListener("click",e=>{e.stopPropagation();deleteSchedNotif(pid,el.dataset.schedDel);});
  });
}
...
function renderPhotosInProg(pid){
  ...
  html+=sets.map(set=>{
    ...
    return`<div class="card" style="margin-bottom:10px"><div class="card-hd"><div class="card-ico">${icon('camera')}</div><div><div class="card-title" style="font-size:13px">${_el(set.note)}</div> <!-- FIX Bug36 --><div class="card-sub">${d.getDate()} ${M[d.getMonth()]} ${d.getFullYear()}</div></div></div><div class="card-bd"><div class="photo-strip">${thumbs}</div></div></div>`;
  }).join("");
  wrap.innerHTML=html;
}
