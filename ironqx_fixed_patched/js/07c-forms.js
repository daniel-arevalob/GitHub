...
function confirmDelete(){
  const pt=DB.pt(S.selPid);if(!pt)return;
  if(confirm(`¿Eliminar a ${pt.name}?\n\nSe borrarán todos sus datos locales.`)){
    const pid=S.selPid;
    // FIX Bug38: limpiar TODOS los datos huérfanos del paciente en localStorage
    // 1. Quitar del array de pacientes
    DB.savePts(DB.pts().filter(p=>p.id!==pid));
    // 2. Limpiar claves separadas de plan e historial
    STORAGE.remove("iq_plan_"+pid);
    STORAGE.remove("iq_planhist_"+pid);
    STORAGE.remove("iq_pin_"+pid);
    STORAGE.remove("iq_goal_"+pid);
    // 3. Limpiar datos dentro de iq_db (prog, adh, reports, notifs, photos, macros, schedNotifs, ach, fichaHistory)
    const db=DB.get();
    ["prog","adh","reports","notifs","photos","macros","schedNotifs","ach","fichaHistory"].forEach(k=>{
      if(db[k]&&db[k][pid])delete db[k][pid];
    });
    DB.save(db);
    // 4. Eliminar en Supabase si está activo
    if(typeof _queueSync==="function"&&typeof _deletePatient==="function"){
      _queueSync(()=>_deletePatient(pid));
    }
    backAdmin();
...
function calcStreak(adh){
  ...
  // FIX Bug33: normalizar valores de adh por si tienen formato objeto tras sync Supabase
  const _ns=v=>{if(!v)return"none";if(typeof v==="string")return v;if(typeof v==="object")return v.notes||"none";return"none";};
  ...
  for(let i=days.length-1;i>=0;i--){
    const key=days[i],val=_ns(adh[key]);
...
function renderMonthHeatmap(pid){
  ...
  // FIX Bug41: normalizar valores de adh por si vienen como objetos tras sync Supabase
  const _nmh=v=>{if(!v)return'none';if(typeof v==='string')return v;if(typeof v==='object')return v.notes||'none';return'none';};
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    const st=_nmh(adh[dateStr]||'none');
