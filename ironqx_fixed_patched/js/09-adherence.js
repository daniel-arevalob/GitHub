// FIX Bug26: normalizar valores de adh que podrían ser objetos tras sync Supabase
function _na(v){if(!v)return'none';if(typeof v==='string')return v;if(typeof v==='object')return v.notes||'none';return'none';}
...
function renderAdh(pid){
  ...
  for(let i=0;i<7;i++){
    ...
    const key=d.toISOString().slice(0,10),st=_na(adh[key]||"none"); // FIX Bug26
    const isFuture=key>todayStr;
    if(!isFuture&&st==="done")done++;
    const cls=(isFuture?"future ":(st==="done"?"done":st==="part"?"part":st==="miss"?"miss":""));
    // FIX Bug28: usar data-key en lugar de onclick inline con pid interpolado
    html+=`<div class="adh-d ${cls}"${isFuture?"":` data-adh-key="${key}"`} ><span class="adh-n">${d.getDate()}</span><span>${dayLabels[d.getDay()]}</span></div>`;
  }
  grid.innerHTML=html;
  // FIX Bug28: delegación de eventos segura — pid nunca sale al HTML
  grid.onclick=e=>{
    const el=e.target.closest("[data-adh-key]");
    if(el)cycleAdh(pid,el.dataset.adhKey,el);
  };
...
function cycleAdh(pid,key,el){
  ...
  for(let i=0;i<7;i++){
    ...
    if(k<=todayStr&&_na(adh[k])==="done")cnt++; // FIX Bug26
  }
