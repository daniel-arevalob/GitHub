...
function drawDonut(pid){
  ...
  // FIX Bug26: normalizar por si sync Supabase dejó objetos en lugar de strings
  const _nv=v=>{if(!v)return'none';if(typeof v==='string')return v;if(typeof v==='object')return v.notes||'none';return'none';};
  const nEntries=entries.map(_nv);
  const done=nEntries.filter(v=>v==="done").length;
  const part=nEntries.filter(v=>v==="part").length;
  const miss=nEntries.filter(v=>v==="miss").length;
