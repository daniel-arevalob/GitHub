...
function loadPatient(pid){
  ...
  G("st-peso").textContent=pesoVal!=null?pesoVal.toFixed(1):"—";
  G("st-dias").textContent=diasVal;
  G("st-sem").textContent=week;
  setTimeout(()=>{
    if(pesoVal!=null)countUp(G("st-peso"),pesoVal,1); // FIX: guard null
  ...
  (function(){
    if(pay.status==="off"){
      ...
      const dl=Math.ceil((parseDateLocal(pt.expDate)-new Date())/864e5);
      const isWarn=dl<=14;
...
async function _refreshPlanFromCloud(pid){
  try{
    ...
    // Adherencia (adh)
    // FIX BugAdh: MERGE en lugar de REEMPLAZAR — evita borrar marcas locales recientes.
    // El paciente puede marcar un día y 900ms después el refresh de Supabase traería datos
    // desactualizados, pisando la marca local. Solución: los datos de Supabase solo se aplican
    // a claves que NO existen ya en local (semanas pasadas sin registro local).
    // La semana actual se preserva completamente desde local para no perder clics.
    ...
    if(adhRes.data&&adhRes.data.length){
      ...
      adhRes.data.forEach(a=>{
        if(a.notes&&(a.notes==="done"||a.notes==="miss"||a.notes==="part")){
          remoteAdh[a.week_key]=a.notes;
        } else {
          remoteAdh[a.week_key]={training:a.training,nutrition:a.nutrition,sleep:a.sleep,steps:a.steps,notes:a.notes};
        }
      });
      // Calcular rango de semana actual (Lun→Dom) para protegerlo
      const _tAdh=new Date(),_dowAdh=_tAdh.getDay();
      const _monAdh=new Date(_tAdh);_monAdh.setDate(_tAdh.getDate()-(_dowAdh===0?6:_dowAdh-1));_monAdh.setHours(0,0,0,0);
      const _sunAdh=new Date(_monAdh);_sunAdh.setDate(_monAdh.getDate()+6);
      const _monStr=_monAdh.toISOString().slice(0,10);
      const _sunStr=_sunAdh.toISOString().slice(0,10);
      const localAdh=(db.adh&&db.adh[pid])||{};
      // Merge: remoto para semanas pasadas, local para semana actual
      const mergedAdh={...remoteAdh};
      Object.keys(localAdh).forEach(k=>{
        // Conservar cualquier clave local de la semana actual o más reciente que el remoto
        if(k>=_monStr&&k<=_sunStr){mergedAdh[k]=localAdh[k];}
        // También conservar claves locales que no existen en remoto (no subidas aún)
        else if(!(k in remoteAdh)){mergedAdh[k]=localAdh[k];}
      });
      const oldAdh=JSON.stringify(localAdh);
      if(JSON.stringify(mergedAdh)!==oldAdh){
        if(!db.adh)db.adh={};
        db.adh[pid]=mergedAdh;
        adhChanged=true;
      }
    }
    ...
    if(_tab==="prog"||metricsChanged||patientChanged){
