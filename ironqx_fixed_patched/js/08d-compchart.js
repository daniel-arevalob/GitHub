...
function drawCompChart(pid, opts){
  ...
  // FIX: usar historia unificada (fichaHistory + prog) para que la tabla
  // aparezca incluso cuando solo hay registros de peso sin medición clínica
  var hist = _buildCompositionHistory(pid);
  var card=G(cardId),sl=G(slId),list=G(listId);
  if(!card||!sl||!list)return;

  // Mostrar la sección si hay al menos 1 entrada con peso O grasa
  var valid=hist.filter(function(h){return parseFloat(h.grasa)||parseFloat(h.peso);});
  ...
  var chrono=valid.slice().sort(function(a,b){return new Date(a.date)-new Date(b.date);}); // oldest first always
  var n=chrono.length;

  /* ── Draw % Grasa chart (solo si hay >=2 entradas con grasa) ── */
  var gcValid=chrono.filter(function(h){return parseFloat(h.grasa);});
  var canvas=G(gcCanvasId);
  if(canvas&&gcValid.length>=2){
    ...
    if(filled.length>=2){
      ...
      /* stats */
      var gcFirst=filled[0], gcLast=filled[filled.length-1], gcDiff=parseFloat((gcLast-gcFirst).toFixed(1));
      var gs=G(statsS), gd=G(statsD), gc_=G(statsC);
      if(gs)gs.textContent=gcFirst.toFixed(1)+"%";
      if(gc_)gc_.textContent=gcLast.toFixed(1)+"%";
      if(gd){gd.textContent=(gcDiff>0?"+":"")+gcDiff+"%"; gd.style.color=gcDiff<=0?"var(--green)":"var(--red)";}
    }
  }

  /* ── Tabla cruzada: Peso · %GC · Masa Grasa · Masa Magra ── */
  ...
  var _pesoLB=weightGoalDir(_admPt&&_admPt.goal||"",_admPt&&_admPt.ficha)==="up"?false:true;
  ...
  chrono.forEach(function(h,i){
    ...
    // Indicador de fuente: 📋 = medición clínica, ⚖️ = solo peso
    var srcTag=h._fromProg&&!h.grasa?'<span style="font-size:8px;opacity:.4;margin-left:2px">⚖</span>':'';
    rows+="<div class='snap-row"+(isLast?" snap-latest":"")+"'>";
    rows+="<div class='fxs tm'>"+d.getDate()+" "+M[d.getMonth()]+" "+String(d.getFullYear()).slice(2)+srcTag+"</div>";
