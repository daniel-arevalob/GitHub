// FIX Bug25: helper de escape HTML para vista del paciente
function _er(s){if(s==null)return"";return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");}
          ...
          return`<div style="border:1px solid var(--line);border-radius:var(--rs);overflow:hidden;margin-bottom:8px">
            <div style="display:flex;align-items:center;gap:10px;padding:10px 13px;cursor:pointer;background:var(--bg2)" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none';this.querySelector('.rh-arr').style.transform=this.nextElementSibling.style.display==='none'?'':'rotate(180deg)'">
              ...
              <div style="flex:1">
                ...
                <div style="font-size:10px;color:var(--muted);font-family:'Inter',sans-serif;margin-top:1px">${or.adherencePct!=null?or.adherencePct:"—"}% adh${or.late?" · Tardío":""}${or.drReply?" · Respondido":""}</div>
              </div>
            ...
            <div style="display:none;padding:10px 13px;background:var(--bg3);border-top:1px solid var(--line)">
              ...
              <div style="font-size:12px;color:var(--txt2);line-height:1.65;margin-bottom:8px">${_er(or.comment)}</div>
              ${or.drReply?`<div class="dr-reply" style="margin-top:0"><div class="dr-reply-hd">${icon('stethoscope','sm')} Dr. Arévalo</div><div class="dr-reply-txt">${_er(or.drReply)}</div></div>`:`<div style="font-size:11px;color:var(--muted);font-family:'Inter',sans-serif">Sin respuesta aún</div>`}
            </div>
          </div>`;
        }).join("")}
      </div>`:"";
...
function sentReportHTML(r,week,status,pid){
  ...
  return`
    <div class="report-banner ${status}" style="margin-bottom:16px">
      ...
      <div class="rb-header"><div class="rb-icon">${r.drReply?icon("chat"):icon("check-circle")}</div><div><div class="rb-title">Reporte Semana ${week}</div><div class="rb-sub">Enviado el ${d.getDate()} ${M[d.getMonth()]} · ${r.adherencePct!=null?r.adherencePct:"—"}% adherencia</div></div></div>
    </div>
    ${weightBlock}
    <div class="report-section">
    ...
    <div class="report-section">
      ...
      <div class="card"><div class="card-bd"><div style="font-size:13px;color:var(--txt2);line-height:1.7">${_er(r.comment)}</div></div></div>
    </div>
    ${r.drReply?`<div class="report-section"><div class="report-section-title" style="color:var(--gold)">${icon('stethoscope','sm')} Respuesta del Dr. Arévalo</div><div class="dr-reply"><div class="dr-reply-hd">${icon('stethoscope','sm')} Dr. Arévalo</div><div class="dr-reply-txt">${_er(r.drReply)}</div></div></div>`:`<div style="text-align:center;padding:16px 0"><div class="ei" style="font-size:28px;margin-bottom:8px;opacity:1">${icon('hourglass','muted')}</div><div class="fsm tm">El Dr. revisará tu reporte pronto</div></div>`}
