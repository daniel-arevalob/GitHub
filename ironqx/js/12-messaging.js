...
function renderInbox(){
  ...
  wrap.innerHTML=pending.map(item=>{
    ...
    return`<div class="report-card" style="margin-bottom:10px"><div class="report-card-hd" onclick="openReplyFromInbox('${item.pid}',${item.idx})">
      <div class="rc-dot ${r.late?"late":"unread"}"></div>
      <div class="f1"><div style="font-size:14px;font-weight:700">${item.ptName}</div><div class="fxs tm mt4">Semana ${r.week} · ${d.getDate()} ${M[d.getMonth()]}${r.late?" · Tardío":""}</div></div>
...
function openReplyFromInbox(pid,weekKeyOrIdx){
  ...
  G("reply-report-preview").innerHTML=`
    <div class="sg c3" style="margin-bottom:12px">
      <div class="sc"><div style="font-family:'Barlow Condensed',sans-serif;font-size:24px;font-weight:900;color:var(--gold)">${r.adherencePct}%</div><div class="su">adherencia</div></div>
      <div class="sc ${r.adherencePct>=70?"gr":r.adherencePct>=40?"am":"rd"}"><div style="font-family:'Barlow Condensed',sans-serif;font-size:24px;font-weight:900">${Object.values(r.adherence).filter(v=>v==="done").length}/7</div><div class="su">días completos</div></div>
