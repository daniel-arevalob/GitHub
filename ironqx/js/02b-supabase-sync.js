...
async function supaLoadAll(coachEmail) {
  ...
  try {
    ...
    if (adh) {
      db.adh = {};
      adh.forEach(a => {
        if (!db.adh[a.patient_id]) db.adh[a.patient_id] = {};
        db.adh[a.patient_id][a.week_key] = { training: a.training, nutrition: a.nutrition, sleep: a.sleep, steps: a.steps, notes: a.notes };
      });
    }
...
async function supaLoadPatientData(pid) {
  ...
  try {
    ...
    if (!aErr && adh && adh.length) {
      ...
      db2.adh[pid] = {};
      adh.forEach(a => { db2.adh[pid][a.week_key] = { training: a.training, nutrition: a.nutrition, sleep: a.sleep, steps: a.steps, notes: a.notes }; });
      DB.save(db2);
    }
    // Notificaciones
