...
function refreshAdmin(){
  ...
  wrap.innerHTML=pts.map(p=>{
    ...
    const adhDone=Object.values(adh).filter(v=>v==="done").length;
    const adhTotal=Math.max(Object.values(adh).length,1);
    ...
    const lastActive=allAdhKeys.find(k=>adh[k]==="done"||adh[k]==="part");
    let actLabel="",actCls="";
    if(lastActive){
